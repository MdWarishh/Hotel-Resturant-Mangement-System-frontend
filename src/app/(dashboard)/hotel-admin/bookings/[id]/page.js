...
export default function BookingDetailsPage() {
  ...
  const handleDownloadPDF = () => {
    ...
    <div class="totals-row">
      ...
      <span class="val" style="color:#7c3aed;">&#8377;${Number(c.amount).toLocaleString('en-IN')}</span>
    </div>`).join('');

    const durationLabel = isHourly
      ? `${nightsCount} Hour${nightsCount !== 1 ? 's' : ''}`
...
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  .totals-box { width:320px; border:1.5px solid #111; }
...
<div class="info-section">
  ...
  <div class="col">
    ...
    <div class="info-row"><span class="info-label">Type</span><span class="info-value">${isHourly ? 'Hourly Stay' : 'Daily Stay'}</span></div>
    <div class="info-row"><span class="info-label">${isHourly ? 'Duration' : 'Nights'}</span><span class="info-value">${durationLabel}</span></div>
...
<table>
  <thead>
    <tr>
      <th>Description</th>
      <th>${isHourly ? 'Duration' : 'Nights'}</th>
      <th style="text-align:right;padding-right:12px;">Amount</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td>Room ${booking.room?.roomNumber || ''} - ${booking.room?.roomType || 'Room'} Charges${booking.pricing?.manualDailyRate > 0 ? ' <span style="font-size:11px;color:#0f766e;font-weight:600;">(Custom Rate)</span>' : ''}</td>
      <td>${durationLabel}</td>
...
<div class="totals-section">
  <div class="totals-box">
    ...
    ${extraCharges > 0 ? `<div class="totals-row"><span class="lbl">Extra Guest Charges</span><span class="val">&#8377;${extraCharges.toLocaleString('en-IN')}</span></div>` : ''}
    ${customChargesSum > 0 ? `
    <div class="totals-row" style="background:#faf5ff;">
      <span class="lbl" style="color:#7c3aed;font-weight:700;">Extra Charges</span>
      <span class="val" style="color:#7c3aed;">&#8377;${customChargesSum.toLocaleString('en-IN')}</span>
    </div>
    ${customChargesTotalsRows}` : ''}
          ...
          <Section title="Room & Stay Details" icon={<DoorOpen className="h-6 w-6" />}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              <div>
                ...
                <Info
                  label={booking.bookingType === 'hourly' ? 'Duration' : 'Nights'}
                  value={booking.bookingType === 'hourly' ? `${booking.hours} Hour${booking.hours > 1 ? 's' : ''}` : nights}
                />
              </div>
            ...
            <div className="text-black space-y-4 text-lg">
              ...
              <div className="flex justify-between items-center">
                <span className="flex items-center gap-2">
                  Room Charges
                  {booking.pricing?.manualDailyRate > 0 && (
                    <span className="text-xs px-2 py-0.5 bg-teal-100 text-teal-700 rounded-full font-semibold">Custom Rate</span>
                  )}
                  {booking.bookingType === 'hourly' && booking.pricing?.manualHourlyRate > 0 && (
                    <span className="text-xs px-2 py-0.5 bg-orange-100 text-orange-700 rounded-full font-semibold">Custom Rate</span>
                  )}
                </span>
              ...
              {booking.pricing?.extraCharges > 0 && (
                <div className="flex justify-between text-teal-600">
                  ...
                  <span>+₹{booking.pricing.extraCharges}</span>
                </div>
              )}
              ...
              {(booking.pricing?.customCharges || []).length > 0 && (
                <div className="bg-purple-50 border border-purple-200 rounded-xl p-4 space-y-2">
                  <p className="text-sm font-semibold text-purple-700 uppercase tracking-wide flex items-center gap-2 mb-3">
                    <Tag className="h-4 w-4" /> Extra Charges
                  </p>
                  {booking.pricing.customCharges.map((c, i) => (
                    <div key={i} className="flex justify-between text-base text-purple-800">
                      <span>{c.label}</span>
                      <span className="font-semibold">+₹{Number(c.amount).toLocaleString()}</span>
                    </div>
                  ))}
                  <div className="border-t border-purple-300 pt-2 flex justify-between text-base font-bold text-purple-900">
